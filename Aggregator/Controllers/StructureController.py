...
from Aggregator.Model.Structure import Structure, StructureDB
...
class StructureController:
    ...
    def get_structure_tree(self) -> list[Structure]: #получение иерархии (дерева) подразделений для фильтра
    ...
    def get_all_structures(self) -> list[Structure]:
        ...
        try:
            ...
            self._logger.error(f"Ошибка при получении структур из БД: {e}")
            return []
        finally:
            session.close()
